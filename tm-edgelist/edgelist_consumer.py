...
parser.add_argument('--system', help='Tracing system used', choices=[
                    'cadets', 'clearscope', 'fivedirections', 'marple', 'theia', 'trace'], required=True)
...
def labelgen(values):
    ...
    if isinstance(values, dict):
        hasher.update(values['type'].encode('utf-8'))
...
def process_cdm_srcsink(record_value, input_format, nid):
    """Process CDM record typed CDM20_TYPE_SRCSINK.
    values = {'nid', 'type'}
    """
    values = dict()
    values['nid'] = nid
    ...
    if 'type' not in record_value:
        raise KeyError('CDM20_TYPE_SRCSINK: type is missing.')
    ...
    # clearscope
        ...
        pass
        # raise KeyError('CDM20_TYPE_SRCSINK: type is undefined.')

    return values


def process_cdm_subject(record_value, input_format, nid):
    """Process CDM record typed CDM20_TYPE_SUBJECT.
    values = {'nid', 'type'}
    """
    values = dict()
    values['nid'] = nid
    ...
    if 'type' not in record_value:
        raise KeyError('CDM20_TYPE_SUBJECT: type is missing.')
    type_value = read_field(record_value['type'], input_format)
    values['type'] = type_value
    ...
    # trace
        ...
        pass
        # raise KeyError('CDM20_TYPE_SUBJECT: type is undefined.')

    return values


def process_cdm_file(record_value, input_format, nid):
    """Process CDM record typed CDM20_TYPE_FILE.
    values = {'nid', 'uuid', type'}
    """
    values = dict()
    values['nid'] = nid
    # we store UUID to detect identical records
    values['uuid'] = record_value['uuid']
    if 'type' not in record_value:
        raise KeyError('CDM20_TYPE_FILE: type is missing.')
    type_value = read_field(record_value['type'], input_format)
    values['type'] = type_value
    ...
    # trace
        ...
        pass
        # raise KeyError('CDM20_TYPE_FILE: type is undefined.')

    return values


def process_cdm_sock(record_value, input_format, nid):
    """Process CDM record typed CDM20_TYPE_SOCK.
    values = {'nid', 'type', 'localAddress',
    ...
    if 'localAddress' not in record_value:
        logging.debug('CDM20_TYPE_SOCK: localAddress is missing.')
    ...
    if 'remotePort' not in record_value:
        logging.debug('CDM20_TYPE_SOCK: remotePort is missing.')
...
def process_cdm_pipe(record_value, input_format, nid):
    """Process CDM record type CDM20_TYPE_PIPE.
    ...
    if 'sourceUUID' not in record_value:
        logging.debug('CDM20_TYPE_PIPE: sourceUUID is missing.')
...
def process_cdm_event(record_value, input_format):
    """Process CDM record typed CDM20_TYPE_EVENT.
    ...
    if 'type' not in record_value:
        logging.debug('CDM20_TYPE_EVENT: type is missing. Event UUID: ' +
                      repr(record_value['uuid']))
    type_value = read_field(record_value['type'], input_format)
    values['type'] = type_value
    values['srcUUID'] = None
    values['dstUUID'] = None
    ...
    if 'timestampNanos' not in record_value:
        logging.debug('CDM20_TYPE_EVENT: timestamp is missing. Event UUID: ' +
                      repr(record_value['uuid']))
    ...
    if type_value == 'EVENT_CLOSE':
        ...
        if type(subject).__name__ == 'NoneType':
            logging.debug("EVENT_CLOSE: subject does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            subjectUUID = subject[CDM20_UUID]
            values['srcUUID'] = subjectUUID

        object1 = record_value['predicateObject']
        if type(object1).__name__ == 'NoneType':
            logging.debug("EVENT_CLOSE: object does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            object1UUID = object1[CDM20_UUID]
        ...
        subject = record_value['subject']
        if type(subject).__name__ == 'NoneType':
            logging.debug("EVENT_FCNTL: subject does not exist. Event ID: " +
                          repr(record_value['uuid']))
        ...
        object1 = record_value['predicateObject']
        if type(object1).__name__ == 'NoneType':
            logging.debug("EVENT_FCNTL: object does not exist. Event ID: " +
                          repr(record_value['uuid']))
        ...
        subject = record_value['subject']
        if type(subject).__name__ == 'NoneType':
            logging.debug("EVENT_CREATE_OBJECT: subject does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            subjectUUID = subject[CDM20_UUID]
            values['srcUUID'] = subjectUUID
        ...
        object1 = record_value['predicateObject']
        if type(object1).__name__ == 'NoneType':
            logging.debug("EVENT_CREATE_OBJECT: object does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            object1UUID = object1[CDM20_UUID]
            values['dstUUID'] = object1UUID
        ...
        subject = record_value['subject']
        if type(subject).__name__ == 'NoneType':
            logging.debug("EVENT_ACCEPT: subject does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            subjectUUID = subject[CDM20_UUID]
        ...
        object1 = record_value['predicateObject']
        if type(object1).__name__ == 'NoneType':
            logging.debug("EVENT_ACCEPT: object does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            object1UUID = object1[CDM20_UUID]
        ...
        subject = record_value['subject']
        if type(subject).__name__ == 'NoneType':
            logging.debug("EVENT_FORK: subject does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            subjectUUID = subject[CDM20_UUID]
            values['srcUUID'] = subjectUUID
        ...
        object1 = record_value['predicateObject']
        if type(object1).__name__ == 'NoneType':
            logging.debug("EVENT_FORK: object does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            object1UUID = object1[CDM20_UUID]
            values['dstUUID'] = object1UUID
        ...
        subject = record_value['subject']
        if type(subject).__name__ == 'NoneType':
            logging.debug("EVENT_OPEN: subject does not exist. Event ID: " +
                          repr(record_value['uuid']))
        ...
        object1 = record_value['predicateObject']
        if type(object1).__name__ == 'NoneType':
            logging.debug("EVENT_OPEN: object does not exist. Event ID: " +
                          repr(record_value['uuid']))
        ...
        subject = record_value['subject']
        if type(subject).__name__ == 'NoneType':
            logging.debug("EVENT_READ: subject does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            subjectUUID = subject[CDM20_UUID]
            values['dstUUID'] = subjectUUID
        ...
        object1 = record_value['predicateObject']
        if type(object1).__name__ == 'NoneType':
            logging.debug("EVENT_READ: object does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            object1UUID = object1[CDM20_UUID]
            values['srcUUID'] = object1UUID
        ...
        subject = record_value['subject']
        if type(subject).__name__ == 'NoneType':
            logging.debug("EVENT_LSEEK: subject does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            subjectUUID = subject[CDM20_UUID]
            values['srcUUID'] = subjectUUID
        ...
        object1 = record_value['predicateObject']
        if type(object1).__name__ == 'NoneType':
            logging.debug("EVENT_LSEEK: object does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            object1UUID = object1[CDM20_UUID]
            values['dstUUID'] = object1UUID
        ...
        # TODO:
        # Did not see CDM20_TYPE_PRINCIPAL
        # Subject -> Object
        subject = record_value['subject']
        if type(subject).__name__ == 'NoneType':
            logging.debug("EVENT_CHANGE_PRINCIPAL: subject does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            subjectUUID = subject[CDM20_UUID]
            values['srcUUID'] = subjectUUID
        ...
        object1 = record_value['predicateObject']
        if type(object1).__name__ == 'NoneType':
            logging.debug("EVENT_CHANGE_PRINCIPAL: object does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            object1UUID = object1[CDM20_UUID]
            values['dstUUID'] = object1UUID
        ...
        subject = record_value['subject']
        if type(subject).__name__ == 'NoneType':
            logging.debug("EVENT_LOGIN: subject does not exist. Event ID: " +
                          repr(record_value['uuid']))
        ...
        object1 = record_value['predicateObject']
        if type(object1).__name__ == 'NoneType':
            logging.debug("EVENT_LOGIN: object does not exist. Event ID: " +
                          repr(record_value['uuid']))
        ...
        subject = record_value['subject']
        if type(subject).__name__ == 'NoneType':
            logging.debug("EVENT_MODIFY_PROCESS: subject does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            subjectUUID = subject[CDM20_UUID]
            values['srcUUID'] = subjectUUID
        ...
        object1 = record_value['predicateObject']
        if type(object1).__name__ == 'NoneType':
            logging.debug("EVENT_MODIFY_PROCESS: object does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            object1UUID = object1[CDM20_UUID]
            values['dstUUID'] = object1UUID
        ...
        subject = record_value['subject']
        if type(subject).__name__ == 'NoneType':
            logging.debug("EVENT_EXECUTE: subject does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            subjectUUID = subject[CDM20_UUID]
            values['srcUUID'] = subjectUUID
        ...
        object1 = record_value['predicateObject']
        if type(object1).__name__ == 'NoneType':
            logging.debug("EVENT_EXECUTE: object does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            object1UUID = object1[CDM20_UUID]
            values['dstUUID'] = object1UUID
        ...
        subject = record_value['subject']
        if type(subject).__name__ == 'NoneType':
            logging.debug("EVENT_MMAP: subject does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            subjectUUID = subject[CDM20_UUID]
            values['srcUUID'] = subjectUUID
        ...
        object1 = record_value['predicateObject']
        if type(object1).__name__ == 'NoneType':
            logging.debug("EVENT_MMAP: object does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            object1UUID = object1[CDM20_UUID]
            values['dstUUID'] = object1UUID
        ...
        subject = record_value['subject']
        if type(subject).__name__ == 'NoneType':
            logging.debug("EVENT_CONNECT: subject does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            subjectUUID = subject[CDM20_UUID]
            values['srcUUID'] = subjectUUID
        ...
        object1 = record_value['predicateObject']
        if type(object1).__name__ == 'NoneType':
            logging.debug("EVENT_CONNECT: object does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            object1UUID = object1[CDM20_UUID]
            values['dstUUID'] = object1UUID
        ...
        subject = record_value['subject']
        if type(subject).__name__ == 'NoneType':
            logging.debug("EVENT_SENDTO: subject does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            subjectUUID = subject[CDM20_UUID]
            values['srcUUID'] = subjectUUID
        ...
        object1 = record_value['predicateObject']
        if type(object1).__name__ == 'NoneType':
            logging.debug("EVENT_SENDTO: object does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            object1UUID = object1[CDM20_UUID]
            values['dstUUID'] = object1UUID
        ...
        subject = record_value['subject']
        if type(subject).__name__ == 'NoneType':
            logging.debug("EVENT_RECVFROM: subject does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            subjectUUID = subject[CDM20_UUID]
            values['dstUUID'] = subjectUUID
        ...
        object1 = record_value['predicateObject']
        if type(object1).__name__ == 'NoneType':
            logging.debug("EVENT_RECVFROM: object does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            object1UUID = object1[CDM20_UUID]
            values['srcUUID'] = object1UUID
        ...
        subject = record_value['subject']
        if type(subject).__name__ == 'NoneType':
            logging.debug("EVENT_WRITE: subject does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            subjectUUID = subject[CDM20_UUID]
            values['srcUUID'] = subjectUUID
        ...
        object1 = record_value['predicateObject']
        if type(object1).__name__ == 'NoneType':
            logging.debug("EVENT_WRITE: object does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            object1UUID = object1[CDM20_UUID]
            values['dstUUID'] = object1UUID
        ...
        object1 = record_value['predicateObject']
        if type(object1).__name__ == 'NoneType':
            logging.debug("EVENT_ADD_OBJECT_ATTRIBUTE: object does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            object1UUID = object1[CDM20_UUID]
            values['srcUUID'] = object1UUID
        ...
        if type(object2).__name__ == 'NoneType':
            logging.debug("EVENT_ADD_OBJECT_ATTRIBUTE: object2 does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            object2UUID = object2[CDM20_UUID]
        ...
        subject = record_value['subject']
        if type(subject).__name__ == 'NoneType':
            logging.debug("EVENT_MODIFY_FILE_ATTRIBUTES: subject does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            subjectUUID = subject[CDM20_UUID]
            values['srcUUID'] = subjectUUID
        ...
        object1 = record_value['predicateObject']
        if type(object1).__name__ == 'NoneType':
            logging.debug("EVENT_MODIFY_FILE_ATTRIBUTES: object does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            object1UUID = object1[CDM20_UUID]
            values['dstUUID'] = object1UUID
        ...
        subject = record_value['subject']
        if type(subject).__name__ == 'NoneType':
            logging.debug("EVENT_TRUNCATE: subject does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            subjectUUID = subject[CDM20_UUID]
            values['srcUUID'] = subjectUUID
        ...
        object1 = record_value['predicateObject']
        if type(object1).__name__ == 'NoneType':
            logging.debug("EVENT_TRUNCATE: object does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            object1UUID = object1[CDM20_UUID]
            values['dstUUID'] = object1UUID
        ...
        object1 = record_value['predicateObject']
        if type(object1).__name__ == 'NoneType':
            logging.debug("EVENT_LINK: object1 does not exist. Event ID: " +
                          repr(record_value['uuid']))
        ...
        object2 = record_value['predicateObject2']
        if type(object2).__name__ == 'NoneType':
            logging.debug("EVENT_LINK: object2 does not exist. Event ID: " +
                          repr(record_value['uuid']))
        ...
        subject = record_value['subject']
        if type(subject).__name__ == 'NoneType':
            logging.debug("EVENT_UNLINK: subject does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            subjectUUID = subject[CDM20_UUID]
            values['srcUUID'] = subjectUUID
        ...
        object1 = record_value['predicateObject']
        if type(object1).__name__ == 'NoneType':
            logging.debug("EVENT_UNLINK: object does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            object1UUID = object1[CDM20_UUID]
            values['dstUUID'] = object1UUID
        ...
        subject = record_value['subject']
        if type(subject).__name__ == 'NoneType':
            logging.debug("EVENT_RECVMSG: subject does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            subjectUUID = subject[CDM20_UUID]
            values['dstUUID'] = subjectUUID
        ...
        object1 = record_value['predicateObject']
        if type(object1).__name__ == 'NoneType':
            logging.debug("EVENT_RECVMSG: object does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            object1UUID = object1[CDM20_UUID]
            values['srcUUID'] = object1UUID
        ...
        object1 = record_value['predicateObject']
        if type(object1).__name__ == 'NoneType':
            logging.debug("EVENT_RENAME: object1 does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            object1UUID = object1[CDM20_UUID]
            values['srcUUID'] = object1UUID
        ...
        object2 = record_value['predicateObject2']
        if type(object2).__name__ == 'NoneType':
            logging.debug("EVENT_RENAME: object2 does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            object2UUID = object2[CDM20_UUID]
            values['dstUUID'] = object2UUID
        ...
        subject = record_value['subject']
        if type(subject).__name__ == 'NoneType':
            logging.debug("EVENT_SIGNAL: subject does not exist. Event ID: " +
                          repr(record_value['uuid']))
        ...
        object1 = record_value['predicateObject']
        if type(object1).__name__ == 'NoneType':
            logging.debug("EVENT_SIGNAL: object does not exist. Event ID: " +
                          repr(record_value['uuid']))
        ...
        subject = record_value['subject']
        if type(subject).__name__ == 'NoneType':
            logging.debug("EVENT_MPROTECT: subject does not exist. Event ID: " +
                          repr(record_value['uuid']))
        ...
        object1 = record_value['predicateObject']
        if type(object1).__name__ == 'NoneType':
            logging.debug("EVENT_MPROTECT: object does not exist. Event ID: " +
                          repr(record_value['uuid']))
        ...
        subject = record_value['subject']
        if type(subject).__name__ == 'NoneType':
            logging.debug("EVENT_SENDMSG: subject does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            subjectUUID = subject[CDM20_UUID]
            values['srcUUID'] = subjectUUID
        ...
        object1 = record_value['predicateObject']
        if type(object1).__name__ == 'NoneType':
            logging.debug("EVENT_SENDMSG: object does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            object1UUID = object1[CDM20_UUID]
            values['dstUUID'] = object1UUID
        ...
        subject = record_value['subject']
        if type(subject).__name__ == 'NoneType':
            logging.debug("EVENT_OTHER: subject does not exist. Event ID: " +
                          repr(record_value['uuid']))
        ...
        object1 = record_value['predicateObject']
        if type(object1).__name__ == 'NoneType':
            logging.debug("EVENT_OTHER: object does not exist. Event ID: " +
                          repr(record_value['uuid']))
        ...
        object1 = record_value['predicateObject']
        if type(object1).__name__ == 'NoneType':
            logging.debug("EVENT_FLOWS_TO: object1 does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            object1UUID = object1[CDM20_UUID]
            values['srcUUID'] = object1UUID
        ...
        object2 = record_value['predicateObject2']
        if type(object2).__name__ == 'NoneType':
            logging.debug("EVENT_FLOWS_TO: object2 does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            object2UUID = object2[CDM20_UUID]
            values['dstUUID'] = object2UUID
        ...
        subject = record_value['subject']
        if type(subject).__name__ == 'NoneType':
            logging.debug("EVENT_BIND: subject does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            subjectUUID = subject[CDM20_UUID]
            values['srcUUID'] = subjectUUID
        ...
        object1 = record_value['predicateObject']
        if type(object1).__name__ == 'NoneType':
            logging.debug("EVENT_BIND: object does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            object1UUID = object1[CDM20_UUID]
            values['dstUUID'] = object1UUID
    ...
    # clearscope
        ...
        subject = record_value['subject']
        if type(subject).__name__ == 'NoneType':
            logging.debug("EVENT_CHECK_FILE_ATTRIBUTES: subject does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            subjectUUID = subject[CDM20_UUID]
            values['dstUUID'] = subjectUUID
        ...
        object1 = record_value['predicateObject']
        if type(object1).__name__ == 'NoneType':
            logging.debug("EVENT_CHECK_FILE_ATTRIBUTES: object does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            object1UUID = object1[CDM20_UUID]
            values['srcUUID'] = object1UUID
        ...
        subject = record_value['subject']
        if type(subject).__name__ == 'NoneType':
            logging.debug("EVENT_WRITE_SOCKET_PARAMS: subject does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            subjectUUID = subject[CDM20_UUID]
            values['srcUUID'] = subjectUUID
        ...
        object1 = record_value['predicateObject']
        if type(object1).__name__ == 'NoneType':
            logging.debug("EVENT_WRITE_SOCKET_PARAMS: object does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            object1UUID = object1[CDM20_UUID]
            values['dstUUID'] = object1UUID
        ...
        subject = record_value['subject']
        if type(subject).__name__ == 'NoneType':
            logging.debug("EVENT_READ_SOCKET_PARAMS: subject does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            subjectUUID = subject[CDM20_UUID]
            values['dstUUID'] = subjectUUID
        ...
        object1 = record_value['predicateObject']
        if type(object1).__name__ == 'NoneType':
            logging.debug("EVENT_READ_SOCKET_PARAMS: object does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            object1UUID = object1[CDM20_UUID]
            values['srcUUID'] = object1UUID
    ...
    # fivedirections
        ...
        object1 = record_value['predicateObject']
        if type(object1).__name__ == 'NoneType':
            logging.debug("EVENT_UPDATE: object1 does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            object1UUID = object1[CDM20_UUID]
            values['srcUUID'] = object1UUID
        ...
        object2 = record_value['predicateObject2']
        if type(object2).__name__ == 'NoneType':
            logging.debug("EVENT_UPDATE: object2 does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            object2UUID = object2[CDM20_UUID]
            values['dstUUID'] = object2UUID
        ...
        subject = record_value['subject']
        if type(subject).__name__ == 'NoneType':
            logging.debug("EVENT_LOADLIBRARY: subject does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            subjectUUID = subject[CDM20_UUID]
            values['dstUUID'] = subjectUUID
        ...
        object1 = record_value['predicateObject']
        if type(object1).__name__ == 'NoneType':
            logging.debug("EVENT_LOADLIBRARY: object does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            object1UUID = object1[CDM20_UUID]
            values['srcUUID'] = object1UUID
        ...
        subject = record_value['subject']
        if type(subject).__name__ == 'NoneType':
            logging.debug("EVENT_CREATE_THREAD: subject does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            subjectUUID = subject[CDM20_UUID]
            values['srcUUID'] = subjectUUID
        ...
        object1 = record_value['predicateObject']
        if type(object1).__name__ == 'NoneType':
            logging.debug("EVENT_CREATE_THREAD: object does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            object1UUID = object1[CDM20_UUID]
            values['dstUUID'] = object1UUID
        ...
        subject = record_value['subject']
        if type(subject).__name__ == 'NoneType':
            logging.debug("EVENT_LOGOUT: subject does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            subjectUUID = subject[CDM20_UUID]
            values['srcUUID'] = subjectUUID
        ...
        object1 = record_value['predicateObject']
        if type(object1).__name__ == 'NoneType':
            logging.debug("EVENT_LOGOUT: object does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            object1UUID = object1[CDM20_UUID]
            values['dstUUID'] = object1UUID
    ...
    # theia
        ...
        subject = record_value['subject']
        if type(subject).__name__ == 'NoneType':
            logging.debug("EVENT_CLONE: subject does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            subjectUUID = subject[CDM20_UUID]
            values['srcUUID'] = subjectUUID
        ...
        object1 = record_value['predicateObject']
        if type(object1).__name__ == 'NoneType':
            logging.debug("EVENT_CLONE: object does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            object1UUID = object1[CDM20_UUID]
            values['dstUUID'] = object1UUID
        ...
        object1 = record_value['predicateObject']
        if type(object1).__name__ == 'NoneType':
            logging.debug("EVENT_SHM: object1 does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            object1UUID = object1[CDM20_UUID]
            values['srcUUID'] = object1UUID
        ...
        object2 = record_value['predicateObject2']
        if type(object2).__name__ == 'NoneType':
            logging.debug("EVENT_SHM: object2 does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            object2UUID = object2[CDM20_UUID]
            values['dstUUID'] = object2UUID
    ...
    # trace
        ...
        subject = record_value['subject']
        if type(subject).__name__ == 'NoneType':
            logging.debug("EVENT_UNIT: subject does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            subjectUUID = subject[CDM20_UUID]
            values['srcUUID'] = subjectUUID
        ...
        object1 = record_value['predicateObject']
        if type(object1).__name__ == 'NoneType':
            logging.debug("EVENT_UNIT: object does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            object1UUID = object1[CDM20_UUID]
            values['dstUUID'] = object1UUID
    ...
    # others
        ...
        object1 = record_value['predicateObject']
        if type(object1).__name__ == 'NoneType':
            logging.debug("EVENT_CORRELATION: object1 does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            object1UUID = object1[CDM20_UUID]
            values['srcUUID'] = object1UUID
        ...
        object2 = record_value['predicateObject2']
        if type(object2).__name__ == 'NoneType':
            logging.debug("EVENT_CORRELATION: object2 does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            object2UUID = object2[CDM20_UUID]
            values['dstUUID'] = object2UUID
        ...
        subject = record_value['subject']
        if type(subject).__name__ == 'NoneType':
            logging.debug("EVENT_LOGCLEAR: subject does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            subjectUUID = subject[CDM20_UUID]
            values['srcUUID'] = subjectUUID
        ...
        object1 = record_value['predicateObject']
        if type(object1).__name__ == 'NoneType':
            logging.debug("EVENT_LOGCLEAR: object does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            object1UUID = object1[CDM20_UUID]
            values['dstUUID'] = object1UUID
        ...
        subject = record_value['subject']
        if type(subject).__name__ == 'NoneType':
            logging.debug("EVENT_MOUNT: subject does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            subjectUUID = subject[CDM20_UUID]
            values['srcUUID'] = subjectUUID
        ...
        object1 = record_value['predicateObject']
        if type(object1).__name__ == 'NoneType':
            logging.debug("EVENT_MOUNT: object does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            object1UUID = object1[CDM20_UUID]
            values['dstUUID'] = object1UUID
        ...
        subject = record_value['subject']
        if type(subject).__name__ == 'NoneType':
            logging.debug("EVENT_SERVICEINSTALL: subject does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            subjectUUID = subject[CDM20_UUID]
            values['srcUUID'] = subjectUUID
        ...
        object1 = record_value['predicateObject']
        if type(object1).__name__ == 'NoneType':
            logging.debug("EVENT_SERVICEINSTALL: object does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            object1UUID = object1[CDM20_UUID]
            values['dstUUID'] = object1UUID
        ...
        subject = record_value['subject']
        if type(subject).__name__ == 'NoneType':
            logging.debug("EVENT_STARTSERVICE: subject does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            subjectUUID = subject[CDM20_UUID]
            values['srcUUID'] = subjectUUID
        ...
        object1 = record_value['predicateObject']
        if type(object1).__name__ == 'NoneType':
            logging.debug("EVENT_STARTSERVICE: object does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            object1UUID = object1[CDM20_UUID]
            values['dstUUID'] = object1UUID
        ...
        subject = record_value['subject']
        if type(subject).__name__ == 'NoneType':
            logging.debug("EVENT_UMOUNT: subject does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            subjectUUID = subject[CDM20_UUID]
            values['srcUUID'] = subjectUUID
        ...
        object1 = record_value['predicateObject']
        if type(object1).__name__ == 'NoneType':
            logging.debug("EVENT_UMOUNT: object does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            object1UUID = object1[CDM20_UUID]
            values['dstUUID'] = object1UUID
        ...
        subject = record_value['subject']
        if type(subject).__name__ == 'NoneType':
            logging.debug("EVENT_WAIT: subject does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            subjectUUID = subject[CDM20_UUID]
            values['dstUUID'] = subjectUUID
        ...
        object1 = record_value['predicateObject']
        if type(object1).__name__ == 'NoneType':
            logging.debug("EVENT_WAIT: object does not exist. Event ID: " +
                          repr(record_value['uuid']))
        else:
            object1UUID = object1[CDM20_UUID]
            values['srcUUID'] = object1UUID
...
def process_cdm_principal(record_value, input_format, nid):
    """Process CDM record typed CDM20_TYPE_PRINCIPAL.
    values = {'nid', 'type'}
    """
    values = dict()
    values['nid'] = nid
    ...
    if 'type' not in record_value:
        raise KeyError('CDM20_TYPE_PRINCIPAL: type is missing.')
    type_value = read_field(record_value['type'], input_format)
    values['type'] = type_value
    ...
    if type_value == 'PRINCIPAL_LOCAL':
        ...
        pass
        # raise KeyError('CDM20_TYPE_PRINCIPAL: type is undefined.')

    return values


def process_cdm_host(record_value, input_format, nid):
    """Process CDM record typed CDM20_TYPE_HOST.
    values = {'nid', 'type'}
    """
    values = dict()
    values['nid'] = nid
    ...
    if 'hostType' not in record_value:
        raise KeyError('CDM20_TYPE_HOST: host type is missing.')
    ...
    if type_value == 'HOST_DESKTOP':
        ...
        pass
        # raise KeyError('CDM20_TYPE_HOST: host type is undefined.')

    return values


def process_cdm_memory(record_value, input_format, nid):
    """Process CDM record typed CDM20_TYPE_MEMORY.
    values = {'nid', 'type'}
    """
    values = dict()
    values['nid'] = nid
...
def generate_output(nodes, fh, fp):
    ...
    for line in fh:
        ...
        if cdm_record_type == CDM20_TYPE_EVENT:
            ...
            if uuid in edgeUUID:
                logging.debug(
                    'CDM20_TYPE_EVENT: UUID is not unique. UUID: ' + repr(uuid))
            edgeUUID.add(uuid)
            ...
            if values['srcUUID'] not in nodes:
                ...
                logging.debug('An unmatched source UUID from edge (' +
                              repr(values['uuid']) + ') of type: ' + values['type'] + '.')
                continue
            else:
            ...
            if values['dstUUID'] not in nodes:
                ...
                logging.debug('An unmatched destination UUID from edge (' +
                              repr(values['uuid']) + ') of type: ' + values['type'] + '.')
                continue
            else:
            ...
            f.write(str(srcNode['nid']) + '\t' + str(dstNode['nid']) + '\t' + srcNodeLabel + ":" +
                    dstNodeLabel + ":" + labelgen(values) + ":" + str(values['timestamp']) + "\t" + "\n")

            if values['bidirectional']:
                f.write(str(dstNode['nid']) + '\t' + str(srcNode['nid']) + '\t' + dstNodeLabel + ":" +
                        srcNodeLabel + ":" + labelgen(values) + ":" + str(values['timestamp']) + "\t" + "\n")
...
if input_format == 'avro':
    ...
    # all the dataset files needede to be parsed together
...
for data_file in files:
    with open(os.path.join(input_source, data_file), 'r') as f:
        for line in f:
            ...
            elif cdm_record_type == CDM20_TYPE_EVENT:
                pass
            ...
            # clearscope
                ...
                pass
            # fivedirections
            elif cdm_record_type == CDM20_TYPE_STARTMARKER:
                pass
                ...
                pass
            # trace
            elif cdm_record_type == CDM20_TYPE_UNITDEPENDENCY:
                # TODO
